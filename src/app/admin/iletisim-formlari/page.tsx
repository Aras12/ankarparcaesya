...
export default function AdminIletisimFormlariPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-800 mb-6">İletişim Formları</h1>
      ...
      {selected && (
        <div className="admin-card mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div><span className="font-semibold text-gray-500">Konu:</span> <span className="text-gray-800">{selected.subject || '-'}</span></div>
            <div className="md:col-span-2"><span className="font-semibold text-gray-500">Mesaj:</span><p className="text-gray-800 mt-1 bg-gray-50 p-3 rounded-lg">{selected.message || '-'}</p></div>
            <div><span className="font-semibold text-gray-500">Tarih:</span> <span className="text-gray-800">{new Date(selected.created_at).toLocaleString('tr-TR')}</span></div>
          </div>
      ...
      <div className="admin-card overflow-x-auto">
        {loading ? <p className="text-gray-400">Yükleniyor...</p> : items.length === 0 ? (
          ...
          <table className="admin-table">
            <thead><tr><th>Ad Soyad</th><th>Telefon</th><th>Konu</th><th>Tarih</th><th>Durum</th><th>İşlem</th></tr></thead>
            <tbody>
              {items.map((item) => (

...
export default function AdminHizmetlerPage() {
  ...
  const loadGallery = async (serviceId: string) => {
    const { data } = await supabase.from('service_gallery').select('*').eq('service_id', serviceId).order('sort_order')
    setGallery(data || [])
  }
  ...
  const save = async () => {
    ...
    if (editing.id) {
      const { error } = await supabase.from('services').update(p).eq('id', editing.id)
  ...
  const addGalleryImage = async (url: string) => {
    ...
    await supabase.from('service_gallery').insert([{ service_id: editing.id, image_url: url, sort_order: gallery.length }])
  ...
  const iconOptions = ['home', 'truck', 'package', 'map-pin', 'navigation', 'warehouse', 'box', 'arrow-up', 'shield', 'building']

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Hizmet Yönetimi</h1>
        <button onClick={() => { setEditing({ name: '', slug: '', title: '', meta_description: '', short_description: '', content: '', icon: 'truck', image_url: '', sort_order: 0, is_active: true }); setGallery([]) }} className="admin-btn admin-btn-primary"><Plus size={18} /> Yeni Ekle</button>
      </div>
      ...
      {editing && (
        <div className="admin-card mb-6 border-2 border-blue-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Hizmet Adı *</label><input className="admin-input" value={editing.name || ''} onChange={(e) => u('name', e.target.value)} /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Slug (URL) *</label><input className="admin-input" value={editing.slug || ''} onChange={(e) => u('slug', e.target.value)} placeholder="evden-eve-nakliyat" /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">İkon</label>
              <select className="admin-input" value={editing.icon || 'truck'} onChange={(e) => u('icon', e.target.value)}>
          ...
          <div className="bg-blue-50 rounded-lg p-4 mb-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div><label className="block text-sm font-semibold text-gray-700 mb-1">Meta Başlık (Title Tag)</label><input className="admin-input" value={editing.title || ''} onChange={(e) => u('title', e.target.value)} placeholder="Ankara Evden Eve Nakliyat | Profesyonel Taşımacılık" /></div>
              <div><label className="block text-sm font-semibold text-gray-700 mb-1">Meta Açıklama (Description)</label><textarea className="admin-input" rows={2} value={editing.meta_description || ''} onChange={(e) => u('meta_description', e.target.value)} placeholder="Ankara evden eve nakliyat hizmeti..." /></div>
            </div>
          </div>

          {/* Kısa Açıklama */}
          <div className="mb-4">
            <label className="block text-sm font-semibold text-gray-700 mb-1">Kısa Açıklama (Kartlarda görünür)</label>
            <textarea className="admin-input" rows={2} value={editing.short_description || ''} onChange={(e) => u('short_description', e.target.value)} placeholder="Evinizi komple taşıyoruz..." />
          </div>

          {/* İçerik */}
          <div className="mb-4">
            <label className="block text-sm font-semibold text-gray-700 mb-1">Sayfa İçeriği</label>
            <RichEditor value={editing.content || ''} onChange={(html) => u('content', html)} rows={8} placeholder="Hizmet içerik yazısı..." />
          </div>
          ...
          {editing.id && (
            <div className="bg-gray-50 rounded-lg p-4 mb-4">
              <h3 className="font-bold text-sm text-gray-700 mb-3">🖼️ Hizmet Galerisi</h3>
      ...
      <div className="admin-card overflow-x-auto">
        {loading ? <p className="text-gray-400 py-4">Yükleniyor...</p> : (
          <table className="admin-table">
            <thead><tr><th>#</th><th>Hizmet</th><th>Slug</th><th>İkon</th><th>Durum</th><th>Sıra</th><th>İşlem</th></tr></thead>
            <tbody>
              {items.map((item, i) => (
                <tr key={item.id}>
                  <td>{i + 1}</td>
                  ...
                  <td className="text-gray-500 text-xs">{item.icon}</td>

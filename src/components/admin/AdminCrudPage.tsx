...
interface Field {
  ...
  type: 'text' | 'textarea' | 'html' | 'number' | 'checkbox' | 'image' | 'select'
...
export default function AdminCrudPage({ table, title, fields, defaultValues, orderBy = 'sort_order', orderAsc = true, nameKey }: Props) {
  ...
  const renderField = (field: Field) => {
    ...
    switch (field.type) {
      ...
      case 'number': return <input type="number" className="admin-input" value={val} onChange={(e) => updateField(field.key, parseInt(e.target.value) || 0)} />
      case 'checkbox': return <div className="flex items-center gap-2 pt-2"><input type="checkbox" checked={val ?? false} onChange={(e) => updateField(field.key, e.target.checked)} id={`f-${field.key}`} className="w-4 h-4" /><label htmlFor={`f-${field.key}`} className="text-sm">{field.label}</label></div>
      case 'image': return <ImageUpload value={val} onChange={(url) => updateField(field.key, url)} />
      case 'select': return <select className="admin-input" value={val} onChange={(e) => updateField(field.key, e.target.value)}><option value="">Seçin</option>{field.options?.map(o => <option key={o} value={o}>{o}</option>)}</select>
      default: return <input className="admin-input" value={val} onChange={(e) => updateField(field.key, e.target.value)} placeholder={field.placeholder} />
    }
  }
      ...
      <div className="admin-card overflow-x-auto">
        {loading ? <p className="text-gray-400 py-4">Yükleniyor...</p> : items.length === 0 ? (
          ...
          <table className="admin-table">
            <thead><tr><th>#</th><th>Başlık</th><th>Slug</th><th>Durum</th><th>Sıra</th><th>İşlem</th></tr></thead>
            <tbody>
              {items.map((item, i) => (
                <tr key={item.id} className={editing?.id === item.id ? 'bg-blue-50' : ''}>
                  ...
                  <td><span className={`admin-badge ${(item.is_active ?? item.is_published ?? true) ? 'admin-badge-green' : 'admin-badge-red'}`}>{(item.is_active ?? item.is_published ?? true) ? 'Aktif' : 'Pasif'}</span></td>
